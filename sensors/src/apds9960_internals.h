...
/** Proximity registers */
/**The Proximity Interrupt Threshold Registers set the high and low trigger points for the comparison function which generates an interrupt.
If PDATA, the value generated by proximity channel, crosses below the lower threshold specified, or above the higher threshold, an interrupt may be signaled to the host processor. 
Interrupt generation is subject to the value set in persistence (PERS).*/
#define  PILT           0x89
#define  PIHT           0x8B
...
The time described by PPLEN is the actual signal integration time. 
The LED will be activated slightly longer (typically 1.36 μs) than the integration time. 
The Proximity Pulse Count Register resets to 0x40*/
#define  PPULSE         0x8E
...
*/
#define  CONTROL        0x8F
/**The Configuration Register Two independently enables or disables the saturation interrupts for Proximity and Clear channel.
Saturation Interrupts are cleared by accessing the Clear Interrupt registers at 0xE5, 0xE6 and 0xE7. 
The LED_BOOST bits allow the LDR pin to sink more current above the maximum setting by LDRIVE and GLDRIVE.*/
#define  CONFIG2        0x90
/**The read-only Status Register provides the status of the device.The register is set to 0x04 at power-up.
CPSAT <7> Clear Photodiode Saturation.When asserted, the analog sensor was at the upper end of its dynamic range.
The bit can be de-asserted by sending a Clear channel interrupt command (0xE6 CICLEAR) or by disabling the ADC (AEN=0).
This bit triggers an interrupt if CPSIEN is set.
PGSAT <6> Indicates that an analog saturation event occurred during a previous proximity or gesture cycle. Once set, this bit remains set until cleared by clear proximity interrupt special function
command (0xE5 PICLEAR) or by disabling Prox (PEN=0). This bit triggers an interrupt if PSIEN is set.
...
has become asserted when GMODE transitioned to zero. The bit is reset when FIFO is completely emptied (read).
...
The read sequence must read byte pairs (low followed by high) starting on an even address boundary (0x94, 0x96, 0x98, or 0x9A) inside the RGBC Data Register block.
...
The upper register will read the correct value even if additional ADC integration cycles end between the reading of the lower and upper registers.
When reading register contents, a read of the lower byte data automatically latches the corresponding higher byte data (16 bit latch).
This feature guarantees that the high byte value has not been updated by the ADC between I2C reads. 
In addition, reading CDATAL register not only latches CDATAH but also latches all eight RGBC register simultaneously (64 bit latch).*/
#define  CDATAL         0x94
#define  CDATAH         0x95
#define  RDATAL         0x96
#define  RDATAH         0x97
...
#define  GCONF4         0xAB
/**The GFLVL Register indicates the number of datasets that are currently available in the FIFO for read. 
Reading a complete FIFO dataset (from address 0xFC to 0xFF) constitutes the reduction of the GPENTH register by one.*/
#define  GFLVL          0xAE
/**The GSTATUS Register indicates the operational condition of the gesture state machine.*/
#define  GSTATUS        0xAF

/**Interrupts are cleared by “address accessing” the appropriate register.
This is special I2C transaction consisting of only two bytes: chip address with R/W = 0, followed by a register address.*/
#define  IFORCE         0xE4
#define  PICLEAR        0xE5
#define  CICLEAR        0xE6
#define  AICLEAR        0xE7
